...
тренировок карточками по месяцам и интерактивные графики прогресса. Хостится на Vercel
(статика + serverless API), читает ту же Supabase; бот на Render не меняется. API
аутентифицирует пользователя по подписи Telegram `initData`. Деплой — см.
[docs/webapp-deploy.md](docs/webapp-deploy.md).
