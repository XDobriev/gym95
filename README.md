...
- `/app` и menu-кнопка «Дневник» — открывают **Telegram Mini App** (визуальный просмотр истории и графиков прогресса)

**Mini App «Дневник»** (`webapp/`) — визуальная витрина поверх бота: read-only просмотр
тренировок карточками по месяцам и интерактивные графики прогресса. **Хостится на том же
Render-процессе, что и бот**: HTTP-сервер бота (`src/webapp/server.ts`) отдаёт собранную
статику `webapp/dist` и API `/api/*`, читающий ту же Supabase. Деплоится автоматически по
git push (Render), URL берётся из `RENDER_EXTERNAL_URL` — кнопка «Дневник» и `/app`
включаются сами. API аутентифицирует пользователя по подписи Telegram `initData`.
Опциональный переезд фронта на Vercel (если нужен) — см. [docs/webapp-deploy.md](docs/webapp-deploy.md).
